...
본 프로젝트 결과를 CPC(프로젝트 보고서 챌린지)에서 발표했다. SOI·Metal Gate·Halo Doping 시뮬레이션을 직접 수행한 만큼, 이어진 두 질의에도 직접 답변하며 프로젝트의 판단 기준과 한계를 다시 점검했다.

**Q1. 중간 데이터 중 SS가 과도하게 높은 결과들이 있는데, 현업에서는 쓸 수 없는 소자다. 왜 이런 데이터를 포함했는가?**

A. 이번 프로젝트의 목표는 특정 소자의 완성도가 아니라 **각 공정이 Short Channel Effect를 개선할 수 있는지 여부**를 판단하는 것이었다. 그래서 단독 적용 단계에서는 SS 수치 자체보다 문턱전압이 정상 범위에 있는지, 누설 전류가 비정상적으로 크지 않은지를 중점으로 판단 기준을 잡았다. SS가 높게 나온 중간 데이터들은 그 판단 과정에서 나온 비교군이며, 최종적으로 도출한 SS 89.9 mV/dec는 실사용 가능한 수준이라 판단해 프로젝트를 마무리했다.

**Q2. Metal Gate 부분에 High-k 물질이 함께 적용된 것인가?**

A. 아니다. Metal Gate가 의미를 가지려면 High-k 유전체가 함께 적용되어야 한다는 것은 알고 있었지만, 학기 중 정해진 기간 안에 High-k 적용까지 구현하는 데는 어려움이 있었다. 그래서 우선 Metal Gate 단독으로 Short Channel Effect를 잡을 수 있는지부터 확인했고, 실제로 효과가 없다는 결과를 통해 오히려 **High-k 물질이 왜 필수적인지를 역으로 깨달을 수 있었다.** (심사자로부터 이 역추론 과정 자체가 좋은 접근이라는 피드백을 받았다.)
